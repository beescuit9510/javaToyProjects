...
public class RoomDao {
	...
	private String isBookedRoom = null;

	{
		try (FileReader fr = new FileReader("resources/query.properties");) {
			...
			isBookedRoom = prop.getProperty("isBookedRoom");
	...
	private Room getRoom(ResultSet r) throws SQLException {
		int i = 1;
		return new Room(r.getInt(i++), r.getString(i++), r.getString(i++), r.getString(i++), r.getDate(i++));
	}

	public int bookingRoom(Connection conn, int room_no, int member_no) {
		PreparedStatement state = null;
		PreparedStatement state2 = null;
		ResultSet isBookedRoomR = null;
		int r = -1;

		try {
			state = conn.prepareStatement(isBookedRoom);
			state.setInt(1, room_no);
			isBookedRoomR = state.executeQuery();

			if (isBookedRoomR.next())
				return -1;

			state2 = conn.prepareStatement(bookingRoom);
			state2.setInt(1, room_no);
			state2.setInt(2, member_no);

			r = state2.executeUpdate();

		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			JDBCTemplate.close(state);
			JDBCTemplate.close(state2);
			JDBCTemplate.close(isBookedRoomR);
		}
		return r;
	}
}
